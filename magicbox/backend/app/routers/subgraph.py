...
@router.get("/subgraph/documents")
async def get_subgraph_documents(
    connector: Optional[str] = Query(None, max_length=64),
    connector_name: Optional[str] = Query(None, max_length=256),
) -> dict:
    """List the documents (biz_id) belonging to a connector source/instance.

    Grouped by ``biz_id`` — the unique document key — so a document carried
    by many entities appears exactly once, while distinct documents that
    happen to share a display name stay as separate rows (they filter
    different graphs). The display name prefers the ``semantic_identifier``
    accumulated at the SAME position as the biz_id entry (``biz_id[i]`` ↔
    ``semantic_identifier[i]``); a missing entry falls back to the biz_id
    itself.

    At least one of ``connector`` / ``connector_name`` is required.
    """
    cfg = get_config()
    if not connector and not connector_name:
        raise HTTPException(
            status_code=400,
            detail="At least one of 'connector' or 'connector_name' must be provided",
        )
    label = f"`{escape_label(cfg.workspace)}`"
    labels = label
    if connector:
        ...
        labels += f":`{escape_label(connector)}`"
    conditions = ["size(coalesce(n.biz_id, [])) > 0"]
    if connector_name:
        conditions.append("$connector_name = n.connector_name")
    where = (" WHERE " + " AND ".join(conditions)) if conditions else ""
    ...
    async with get_session() as session:
        rows = await _fetch_with_retry(
            ...
            MATCH (n:{labels})
            {where}
            UNWIND range(0, size(n.biz_id) - 1) AS i
            WITH n.biz_id[i] AS biz_id,
                 coalesce(coalesce(n.semantic_identifier, [])[i], n.biz_id[i]) AS name
            RETURN biz_id, collect(DISTINCT name)[0] AS name, count(*) AS entities
            ORDER BY name
            """,
            connector_name=connector_name,
        )
    ...
    return {
        "documents": [
            {"biz_id": row["biz_id"], "name": row["name"], "entities": row["entities"]}
            for row in rows
        ]
    }
