...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    base: './',
    build: {
      // Magicbox WebUI builds to its own dist/ (no bundling into the
      // original LightRAG API package).
    ...
    server: {
      // Magicbox WebUI dev server port (original WebUI uses 5173).
      port: 5174,
      proxy: {
        // Onyx gateway: strip the /onyx prefix so the onyx magicbox-backend
        // on 8090 sees /connectors, /external-user-groups, ... (see
        // src/api/onyx.ts).
        '/onyx': {
          target: env.VITE_ONYX_GATEWAY_URL || 'http://localhost:8090',
          changeOrigin: true,
          rewrite: (path: string) => path.replace(/^\/onyx/, '')
        },
        // Magicbox query service: strip the /magicbox prefix so the
        // backend on 9622 sees /entities/..., /subgraph, /health.
        '/magicbox': {
          target: env.VITE_MAGICBOX_BACKEND_URL || 'http://localhost:9622',
          changeOrigin: true,
          rewrite: (path: string) => path.replace(/^\/magicbox/, '')
        },
