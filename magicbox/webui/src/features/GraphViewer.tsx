  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      >
        ...
        {/* Top-left filter panel: three aligned rows — subject (labels +
            search), connector (source → instance), permission (ACL). All
            connector/permission widgets are deferred drafts applied via the
            Apply button (see GraphFilterPanel). */}
        <GraphFilterPanel
          searchSlot={showNodeSearchBar && !isThemeSwitching ? <GraphSearch value={searchInitSelectedNode} onFocus={onSearchFocus} onChange={onSearchSelect} /> : undefined}
        />
      ...
      {(isFetching || isThemeSwitching || isLayoutComputing) && (
        <div className="bg-background/80 absolute inset-0 z-10 flex items-center justify-center">
          <div className="text-center">
            ...
            <p>
              {isThemeSwitching
                ? t('graphPanel.overlay.switchingTheme')
                : isFetching
                  ? t('graphPanel.overlay.loadingGraphData')
                  : t('graphPanel.overlay.computingLayout')}
            </p>
          </div>
        </div>
      )}
