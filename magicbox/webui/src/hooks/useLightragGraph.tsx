...
import { checkMagicboxHealth, getSubgraph } from '@/api/magicbox'
...
// Resolve the human-readable display name for a node. Priority: entity_id
// (the stable entity name) → semantic_identifier[0] (the first accumulated
// document display name, e.g. "KEY: summary" for JIRA) → node id. The
// Neo4j workspace label (e.g. "base") is never a display name — it stays
// available in `labels` for the properties panel's label row.
const resolveNodeDisplayName = (
  properties: Record<string, unknown> | undefined,
  fallbackId: string
): string => {
  const entityId = properties?.entity_id
  if (typeof entityId === 'string' && entityId) return entityId
  const si = properties?.semantic_identifier
  if (Array.isArray(si) && si.length > 0 && typeof si[0] === 'string' && si[0]) {
    return si[0]
  }
  if (typeof si === 'string' && si) return si
  return fallbackId
}
...
// The workspace label (e.g. "base") reported by the magicbox /health
// endpoint, fetched ONCE and cached module-wide. Official /graphs payloads
// do not carry it, but it is needed to normalize their labels.
let cachedWorkspaceLabel: string | null = null
let workspaceLabelPromise: Promise<string | null> | null = null

async function getWorkspaceLabel(): Promise<string | null> {
  if (cachedWorkspaceLabel) return cachedWorkspaceLabel
  if (!workspaceLabelPromise) {
    workspaceLabelPromise = checkMagicboxHealth()
      .then((health) => {
        cachedWorkspaceLabel = health.label || null
        return cachedWorkspaceLabel
      })
      .catch((e) => {
        console.error('Failed to fetch magicbox health for workspace label:', e)
        return null
      })
      .finally(() => {
        workspaceLabelPromise = null
      })
  }
  return workspaceLabelPromise
}

// Official /graphs nodes carry labels=[entity_id] (upstream convention),
// while magicbox /subgraph nodes carry the REAL Neo4j labels (workspace +
// connector source, e.g. base/JIRA). Normalize official payloads to the
// real labels so both data sources display one consistent label set. If the
// workspace label is unavailable the payload is left untouched (upstream
// behavior preserved). Nodes already carrying real labels (length != 1 or
// value != entity_id) are skipped.
function normalizeOfficialNodeLabels(rawData: any, workspace: string | null) {
  if (!workspace || !rawData?.nodes) return
  for (const node of rawData.nodes) {
    ...
    if (
      ...
      const source = node.properties?.connector_source
      node.labels = source ? [workspace, source] : [workspace]
    }
  }
}
...
const fetchGraph = async (label: string, maxDepth: number, maxNodes: number) => {
  ...
  // Normalize official labels ([entity_id]) to the REAL Neo4j labels
  // (workspace + connector source) for a display consistent with the
  // magicbox subgraph source.
  const workspaceLabel = await getWorkspaceLabel()
  normalizeOfficialNodeLabels(rawData, workspaceLabel)

  const rawGraph = convertToRawGraph(rawData)

  // console.debug({ data: JSON.parse(JSON.stringify(rawData)) })
  return { rawGraph, is_truncated: rawData?.is_truncated }
}

// Convert a LightragGraphType JSON payload into the RawGraph shape the
// renderer consumes: attach degree/size and build the null-prototype id maps.
// Shared by fetchGraph and fetchMagicboxSubgraph.
const convertToRawGraph = (rawData: any): RawGraph | null => {
  ...
  const converted = new RawGraph()
  converted.nodes = rawData.nodes
  converted.edges = rawData.edges
  converted.nodeIdMap = nodeIdMap
  converted.edgeIdMap = edgeIdMap
...
const createSigmaGraph = async (rawGraph: RawGraph | null): Promise<UndirectedGraph | null> => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    graph.addNode(rawNode.id, {
      label: resolveNodeDisplayName(rawNode.properties, rawNode.id),
      color: rawNode.color,
      x,
      y,
      size: rawNode.size,
...
// Fetch the magicbox connector/ACL-filtered subgraph. Mirrors fetchGraph's
// error contract (rethrow after recording) so the caller's bounded-retry
// path applies unchanged.
const fetchMagicboxSubgraph = async (
  filters: {
    connector: string
  ...
  try {
    console.log(
      `Fetching magicbox subgraph: connector=${filters.connector || '-'}, connectorName=${filters.connectorName || '-'}, bizId=${filters.bizId || '-'}, email=${filters.userEmail || '-'}, groups=${filters.groups || '-'}, publicOnly=${filters.publicOnly}, nonPublicOnly=${filters.nonPublicOnly}`
    )
    const rawData = await getSubgraph(
      null,
      Math.min(maxNodes, 2000),
      Math.min(maxNodes * 2, 4000),
      {
        connector: filters.connector || undefined,
        connectorName: filters.connectorName || undefined,
        ...
        nonPublicOnly: filters.nonPublicOnly || undefined
      }
    )
    return { rawGraph: convertToRawGraph(rawData), is_truncated: undefined }
  } catch (e) {
    useBackendState.getState().setErrorMessage(errorMessage(e), 'Magicbox Subgraph Error!')
    throw e
  }
}
...
const useLightrangeGraph = () => {
  ...
  // Reset graph when query label is cleared (only in the official /graphs
  // data source — a filter-driven magicbox fetch has no query label).
  useEffect(() => {
    const filterActive = !!(
    ...
    if (!filterActive && !queryLabel && (rawGraph !== null || sigmaGraph !== null)) {
  ...
  }, [queryLabel, rawGraph, sigmaGraph, graphFilter])

  // A filter change switches the data source (official /graphs <-> magicbox
  // /subgraph): re-open the fetch gate so the fetch effect re-runs for the
  // new signature instead of being suppressed as a completed duplicate.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // (filter-driven magicbox fetches don't need a query label).
    const filterActive = !!(
      graphFilter.connector ||
      graphFilter.connectorName ||
      graphFilter.bizId ||
    ...
    if (!filterActive && !queryLabel && emptyDataHandledRef.current) {
      return
    }
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      const fetchSignature = `${filterActive ? 'magicbox:' : queryLabel}|${maxQueryDepth}|${maxNodes}|v${graphDataVersion}|${JSON.stringify(graphFilter)}`
      ...
      // 1. A filter is active: fetch the magicbox connector/ACL subgraph.
      if (filterActive) {
        dataPromise = fetchMagicboxSubgraph(graphFilter, currentMaxNodes)
      ...
      dataPromise
        .then(async (result) => {
          ...
          if (!data || !data.nodes || data.nodes.length === 0) {
            ...
            // (filter-driven fetches keep the queryLabel untouched).
            if (!isAuthError && currentQueryLabel && !filterActive) {
  ...
  }, [queryLabel, graphFilter, maxQueryDepth, maxNodes, isFetching, t, graphDataVersion, retryNonce])
  ...
  useEffect(() => {
    ...
    const handleNodeExpand = async () => {
      ...
      try {
        ...
        // The official /graphs API seeds its BFS from an ENTITY ID, not a
        // graph label: it runs `WHERE start.entity_id = $entity_id`. Magicbox
        // nodes carry workspace labels (e.g. "base"), which never match — so
        // pass the node's entity_id property, falling back to the node id
        // (magicbox node ids ARE entity ids).
        const entityId = nodeToExpand.properties?.entity_id || nodeId

        // Fetch the 1-hop neighborhood around the entity
        const extendedGraph = await queryGraphs(entityId, 1, 1000)
        ...
        // The official /graphs API returns node labels as [entity_id]
        // (entity names, an upstream convention), while magicbox /subgraph
        // nodes carry REAL Neo4j labels (workspace + connector source, e.g.
        // base/JIRA). When the expanded node is a magicbox node, stamp its
        // real labels onto the fetched neighbors so the canvas shows one
        // consistent label set (they come from the same Neo4j graph). For
        // official-graph nodes (labels == [entity_id]) the official labels
        // are kept as-is.
        const expandedNodeHasRealLabels = !nodeToExpand.labels.every(
        ...
        for (const node of extendedGraph.nodes) {
          ...
          processedNodes.push({
            id: node.id,
            labels: expandedNodeHasRealLabels ? [...nodeToExpand.labels] : node.labels,
        ...
        for (let newNodeIdx = 0; newNodeIdx < nodesToAddList.length; newNodeIdx++) {
          ...
          sigmaGraph.addNode(nodeId, {
            label: resolveNodeDisplayName(newNode.properties, nodeId),
            color: newNode.color,
            x: x,
            y: y,
            size: nodeSize,
