...
interface SettingsState {
  ...
  // Magicbox graph filter (connector source + ACL). When any field is set,
  // the graph view switches its data source to the magicbox /subgraph API;
  // otherwise the official /graphs endpoint is used.
  graphFilter: {
    connector: string
  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (state: any, version: number) => {
        ...
        if (version < 21) {
          // Add the publicOnly flag to the graph filter
          state.graphFilter = {
            publicOnly: false,
            ...(state.graphFilter ?? { connector: '', userEmail: '', groups: '' })
          }
        }
        if (version < 22) {
          // Default the UI language to Chinese (per the WebUI localization
          // requirement). One-time: users can still switch back in the UI.
          state.language = 'zh'
        }
        if (version < 23) {
          // Add the connectorName (connector instance) field to the graph filter
          state.graphFilter = {
            connectorName: '',
            ...(state.graphFilter ?? {
              connector: '',
        ...
        if (version < 24) {
          // Add the nonPublicOnly visibility option to the graph filter
          state.graphFilter = {
            nonPublicOnly: false,
            ...(state.graphFilter ?? {
              connector: '',
        ...
        if (version < 25) {
          // Add the bizId (document) filter to the graph filter
          state.graphFilter = {
            bizId: '',
            ...(state.graphFilter ?? {
              connector: '',
