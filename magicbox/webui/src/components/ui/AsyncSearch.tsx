...
export function AsyncSearch<T>({
  ...
  return (
    <div
      ref={containerRef}
      // relative: anchors the absolutely-positioned results list (CommandList
      // below) so it overlays content instead of pushing it down in the flow.
    ...
    >
      <Command shouldFilter={false} className="bg-transparent">
        ...
        {/* Floating results list: absolutely positioned below the input so it
            overlays surrounding UI (e.g. the graph filter panel) instead of
            growing the document flow and pushing other rows down. */}
        <CommandList
          hidden={!open}
          className="bg-background absolute top-full right-0 z-50 mt-1 min-w-64 rounded-md border shadow-md max-h-[300px] overflow-x-hidden overflow-y-auto"
        >
