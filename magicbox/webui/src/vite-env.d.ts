...
interface ImportMetaEnv {
  ...
  /* ───────────── magicbox query service ───────────── */

  /** Magicbox backend origin used by `src/api/magicbox.ts` in dev
   *  (e.g. `http://localhost:9622`). Unset in production builds → the
   *  client falls back to the relative `/magicbox` prefix. */
  readonly VITE_MAGICBOX_BACKEND_URL?: string
